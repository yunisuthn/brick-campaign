...
describe('createQueryClient', () => {
  it('never retries a 4xx, retries a network failure or a 5xx once', () => {
    ...
    expect(retry(0, new ApiError(401, { message: 'Unauthorized' }))).toBe(false);
    expect(retry(0, new ApiError(503, { message: 'down' }))).toBe(true);
    expect(retry(1, new ApiError(503, { message: 'down' }))).toBe(false);
