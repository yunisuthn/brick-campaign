...
describe('Moulders (e2e)', () => {
  ...
  it('requires a session', async () => {
    await request(ctx.app.getHttpServer()).get('/api/moulders').expect(401);
  });
  ...
  it('rejects a blank name and a bad query flag with 400', async () => {
    ...
    await request(server)
      ...
      .expect(400);
    await request(server)
      .get('/api/moulders?includeInactive=maybe')
      .set('Cookie', cookie)
      .expect(400);
  });
  ...
  it('creates, reads, retires and hides a retired moulder from the default list', async () => {
    ...
    const read = await request(server).get(`/api/moulders/${id}`).set('Cookie', cookie).expect(200);
    expect(read.body).toEqual(created.body);

    const retired = await request(server)
      .patch(`/api/moulders/${id}`)
      .set('Cookie', cookie)
      .send({ active: false })
      .expect(200);
    expect(retired.body).toEqual({ ...created.body, active: false });

    const active = await request(server).get('/api/moulders').set('Cookie', cookie).expect(200);
    expect(active.body.map((m: { id: string }) => m.id)).not.toContain(id);

    const all = await request(server)
      .get('/api/moulders?includeInactive=true')
      .set('Cookie', cookie)
      .expect(200);
